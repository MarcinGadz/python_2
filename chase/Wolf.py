...
class Wolf:
    ...
    def __init__(self, wolf_move_dist):
        logging.debug(f"Wolf has been initialized with args:"
                      f" wolf_move_dist: {wolf_move_dist}")
        self.wolf_move_dist = wolf_move_dist

    def move(self, nearest_sheep):
        logging.debug(f"executed with args: "
                      f"nearest_sheep: {nearest_sheep.number}")
        (old_x, old_y) = (self.x, self.y)

        dist = math.sqrt(
            (nearest_sheep.x - self.x) ** 2 + (nearest_sheep.y - self.y) ** 2
        )
        ...
        logging.info(f"wolf moved from [{old_x}, {old_y}] "
                     f"to [{self.x}, {self.y}]")
